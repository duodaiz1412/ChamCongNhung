...
async function findAvailableTemplateId() {
    try {
        ...
        console.log(`Currently used IDs in DB: ${[...usedIdSet].sort((a, b) => a - b).join(', ')}`);

        let availableId = 1;

        while (availableId <= MAX_FINGERPRINT_CAPACITY) {
            const isUsedInDb = usedIdSet.has(availableId);
            const isPendingEnroll = websocketService.pendingEnrollment.has(availableId);
        ...
        console.warn(`No available template ID found within capacity (${MAX_FINGERPRINT_CAPACITY}). Sensor might be full or all available IDs are pending.`);
        return null;
...
const getUsers = async (req, res) => {
    try {
        ...
        if (req.query.search) {
            ...
            filter.$or = [ // Tìm trong nhiều trường
                { name: searchRegex },
                { msv: searchRegex },
...
const addUser = async (req, res) => {
    try {
        ...
        if (error.code === 11000) {
            return res.status(409).json({ status: 'error', message: `Duplicate key error. Field: ${Object.keys(error.keyValue)}` });
        }
...
const updateUser = async (req, res) => {
    try {
        ...
        if (error.name === 'ValidationError') {
         ...
         if (error.code === 11000) {
             return res.status(409).json({ status: 'error', message: `Duplicate key error during update. Field: ${Object.keys(error.keyValue)}` });
         }
...
const requestEnrollment = async (req, res) => {
    ...
    if (!websocketService.clients.has(deviceId)) {
         return res.status(400).json({ status: 'error', message: `Device ${deviceId} is not connected.` });
    }

    try {
        // 1. Tìm ID vị trí trống
        const availableId = await findAvailableTemplateId();

        if (availableId === null) {
            return res.status(503).json({ status: 'error', message: 'No available fingerprint slot found or sensor is full.' });
        }

        console.log(`Attempting to start enrollment for user ${name} with template ID ${availableId} on device ${deviceId}`);

        // 2. Gửi yêu cầu tới ESP và chờ phản hồi (Promise sẽ resolve nếu thành công, reject nếu lỗi/timeout)
        const enrollResponse = await websocketService.requestEnrollmentOnDevice(deviceId, availableId);

        // 3. Xử lý kết quả thành công từ ESP
        if (enrollResponse.status === 'success') {
             // userId sẽ tự sinh khi tạo mới
            const newUser = new User({
                name: name,
                msv: msv,
            ...
            res.status(201).json({ status: 'success', message: 'Enrollment successful!', data: newUser });
        }
        // Không cần else, vì nếu không success, promise đã reject và bị bắt bởi catch

    } catch (error) {
        // Bắt lỗi từ findAvailableTemplateId hoặc requestEnrollmentOnDevice
        console.error("Error during enrollment request:", error);
        if (error.message.includes('timed out')) {
           res.status(408).json({ status: 'error', message: `Enrollment timed out: ${error.message}` });
        } else if (error.message.includes('Failed to send enroll command')) {
            res.status(502).json({ status: 'error', message: error.message });
        } else if (error.code === 11000) { // Lỗi trùng key khi save user (ít xảy ra nếu logic findAvailableTemplateId đúng)
            res.status(409).json({ status: 'error', message: `Duplicate key error saving user. Field: ${Object.keys(error.keyValue)}` });
        } else if (error.message.includes('Internal error while finding available template ID')) {
            res.status(500).json({ status: 'error', message: error.message });
        }
        else {
           // Các lỗi khác từ ESP (reject từ requestEnrollmentOnDevice) hoặc lỗi DB khác
           res.status(500).json({ status: 'error', message: error.message || 'Enrollment failed.' });
       }
    }
};
...
const initiateDeleteUser = async (req, res) => {
    ...
    if (!deviceId) {
       return res.status(400).json({ status: 'error', message: 'Device ID is required for deletion.' });
    }
    if (!websocketService.clients.has(deviceId)) {
         return res.status(400).json({ status: 'error', message: `Device ${deviceId} is not connected.` });
    }
    ...
    try {
        ...
        if (!user.id) {
            ...
            await User.findOneAndUpdate({ userId: userIdToDelete }, { $set: { isActive: false } });
            // await User.deleteOne({ userId: userIdToDelete });
            console.log(`User ${userIdToDelete} had no fingerprint assigned. Marked as inactive (or deleted).`);
            return res.json({ status: 'success', message: 'User had no fingerprint assigned. Marked as inactive (or deleted).' });
        }

        const fingerprintTemplateId = user.id;
        console.log(`Attempting to delete fingerprint ID ${fingerprintTemplateId} for user ${user.name} (${userIdToDelete}) on device ${deviceId}`);

        // 2. Gửi yêu cầu xóa tới ESP và chờ phản hồi
        const deleteResponse = await websocketService.requestDeletionOnDevice(deviceId, fingerprintTemplateId);
        ...
        if (deleteResponse.status === 'success') {
            ...
            console.log(`Successfully deleted fingerprint ${fingerprintTemplateId} from device. User ${userIdToDelete} status updated in DB.`);
            res.json({ status: 'success', message: 'Fingerprint deleted successfully from device and user updated.', data: updatedUser });
        }
         // Không cần else, vì nếu không success, promise đã reject và bị bắt bởi catch

    } catch (error) {
        console.error(`Error initiating deletion for user ${userIdToDelete}:`, error);
        if (error.message.includes('timed out')) {
           res.status(408).json({ status: 'error', message: `Deletion timed out: ${error.message}` });
            ...
            res.status(500).json({ status: 'error', message: error.message || 'Deletion failed.' });
        }
    }
};
